...
class WindowInputPad:
    ...
    def __init__(self, device: DeviceController, target_tag):
        self.device = device
    ...
    def draw(self):
        ...
        with dpg.window(
                tag=self.tag_win,
                label=f"InputPad - {self.device.serial_no}", no_resize=True, no_collapse=True, pos=[10, 10],
...
class WindowVideo:
    ...
    def window_label(self) -> str:
        return f"{self.device.serial_no} " + f"{self.dvc.coord_draw.width} x {self.dvc.coord_draw.height}"
    ...
    def _init_tab_control(self):
        ...
        def win_input_pad():
            wpp = WindowInputPad(self.device, self.tag_win_main)
            wpp.draw()
